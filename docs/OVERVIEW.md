...
- **解析 AI**  
  リスクシミュレーションは Vertex AI の Gemini を用いた自律型マルチエージェント（6 カテゴリ並列 + 合成）で実行する。

- **アプリガイド AI**  
  画面上の AI アシスト（フロートボタン）から、アプリの使い方・画面・ボタンの意味・アラート閾値の説明などを質問できる。パネルはドラッグで移動・右下でリサイズ可能。現在の分析結果・リスク数・ToDo 進捗・ピン数・レポート本文（PDF フル版と同じ内容）を渡すと、それらを前提に次のアクション提案や具体的な質問への簡潔な回答（2〜5 文程度）を行う。Gemini 2.5 Flash がアプリガイドに基づいて回答する。
...
- **ローカル開発**: バックエンド（FastAPI）とフロントエンド（Vite）をそれぞれ起動し、環境変数（`.env`）で API キー・Firebase などを設定して利用する。
- **本番デプロイ**: Google Cloud Run に対応。`backend/` と `frontend/` にそれぞれ `Dockerfile` を用意しており、ソースからビルド・デプロイできる。環境変数は Cloud Run の設定またはビルド時の引数で指定する。
