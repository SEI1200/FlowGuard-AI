...
export async function joinProjectByCode(code: string): Promise<ProjectDoc | null> {
  ...
  return {
    ...
    pins: Array.isArray(data.pins) ? (data.pins as Record<string, unknown>[]).map(normalizePin) : [],
    mapTodos: Array.isArray(data.mapTodos) ? (data.mapTodos as Record<string, unknown>[]).map(normalizeMapTodo) : [],
